...
router.post('/:parentId/approveChore', async (req, res, next) => {
    try {
      ...
      if (!choreEntry.approved) {
        // Only increase points if it was not already approved
        child.points += choreEntry.pointsEarned;
      }
  
      choreEntry.approved = true;
      choreEntry.rejected = false;
...
router.post('/:parentId/approveReward', async (req, res, next) => {
    try {
      const { childId, rewardRequestId } = req.body;
  
      const child = await Child.findById(childId);
      if (!child) return 
      next(new Error('Child not found'));
  
      const rewardRequest = child.pendingRewards.id(rewardRequestId);
